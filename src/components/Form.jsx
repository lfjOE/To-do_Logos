...
export default function Form({ addTask }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (task.trim() && (task.trim() !== "")) { // Asegurarse de que la tarea no esté vacía
  ...
  return (
    <form className="TodoForm bg-transparent w-full max-w-4xl" onSubmit={handleSubmit}>
      <input
        className="text-white bg-transparent border-gray-600 border-2 w-full p-3 rounded-md mb-4 line-clamp-3"
        type="text"
        id="task"
